...
<body>
    <header class="topbar">
        <div class="brand">STACKERS · Cổng tuyển sinh</div>
        <a href="../index.php"><small>← Danh sách Use Case</small></a>
    </header>
    <main class="container">
        <h1 class="page-title">Danh sách trường tuyển sinh lớp 10</h1>
        <p class="subtitle">Tra cứu các trường tham gia tuyển sinh và thông tin tuyển sinh liên quan.</p>
            ...
            <div class="grid">
                <div class="col-8">
                    <select name="scenario">
                        <option value="normal" <?= $scenario === 'normal' ? 'selected' : '' ?>>Basic flow - tải danh sách
                            thành công</option>
                        <option value="load_error" <?= $scenario === 'load_error' ? 'selected' : '' ?>>Exception 2.1 - lỗi
                            tải danh sách</option>
                        <option value="detail_error" <?= $scenario === 'detail_error' ? 'selected' : '' ?>>Exception 7.1 -
                            lỗi tải chi tiết</option>
                    </select>
                </div>
        ...
        <?php if ($scenario === 'load_error'): ?>
            <div class="alert alert-danger">Không thể tải danh sách trường, vui lòng thử lại sau.</div>
        <?php else: ?>
            <div class="card">
                <div class="grid">
                    <div class="col-6">
            ...
            <div id="noResult" class="alert alert-warning hidden">Không tìm thấy trường phù hợp.</div>
    ...
    <div id="schoolModal" class="modal">
        <div class="modal-box">
            <div class="modal-header">
                <h2 id="schoolTitle" style="margin:0">Chi tiết trường</h2><button class="modal-close"
                    onclick="closeModal('schoolModal')">✕</button>
            </div>
            <div id="schoolDetail"></div>
            <div class="actions"><button class="btn btn-primary" onclick="closeModal('schoolModal')">Quay lại danh
                    sách</button></div>
        </div>
    </div>
    ...
    <div class="col-4 school-item">
      <div class="school-card">
        <span class="badge ${s.type==='Chuyên'?'badge-yellow':'badge-blue'}">${s.type}</span>
        <h3>${s.name}</h3>
        <p><b>Khu vực:</b> ${s.area}</p>
        <p><b>Địa chỉ:</b> ${s.address}</p>
        <div class="actions"><button class="btn btn-primary" onclick="viewSchool(${i})">Xem chi tiết</button></div>
      </div>
    </div>`).join('');
        ...
        function viewSchool(i) {
            ...
            const s = schools[i];
            document.getElementById('schoolTitle').textContent = s.name;
            document.getElementById('schoolDetail').innerHTML = `
    <table>
      <tr><th>Loại trường</th><td>${s.type}</td></tr>
      <tr><th>Khu vực</th><td>${s.area}</td></tr>
      <tr><th>Địa chỉ</th><td>${s.address}</td></tr>
      <tr><th>Chỉ tiêu</th><td>${s.quota} học sinh</td></tr>
      <tr><th>Phương thức tuyển sinh</th><td>${s.method}</td></tr>
      <tr><th>Thông tin liên quan</th><td>${s.note}</td></tr>
    </table>`;
            openModal('schoolModal');
        }
